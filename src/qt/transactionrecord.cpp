...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZGGHModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zgghTracker->HasMintTx(hash))) {
            //zGGH stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZGGH;
...
bool IsZGGHType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZGGH:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZGGH || type == TransactionRecord::MNReward) {
