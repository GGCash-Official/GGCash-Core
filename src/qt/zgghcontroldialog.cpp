...
#include "ui_zgghcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zgghTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zgghTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zGGH.";
            else if (!mint.isSeedCorrect)
                strReason = "The zGGH seed used to mint this zGGH is not the same as currently hold in the wallet";
            else
