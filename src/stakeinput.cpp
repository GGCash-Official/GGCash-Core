...
// The zGGH block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zGGH is a hash of the serial
...
{
    //Create an output returning the zGGH that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZGGHOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zGGH output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zGGH", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZGGHOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zGGH output", __func__);
...
{
    CzGGHTracker* zgghTracker = pwallet->zgghTracker.get();
    CMintMeta meta;
    if (!zgghTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zgghTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a GGH stake is the outpoint
