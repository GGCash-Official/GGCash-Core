...
public:
    ...
    bool WriteZGGHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGGHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGGHSeed_deprecated(uint256& seed);
    bool EraseZGGHSeed();
    bool EraseZGGHSeed_deprecated();

    bool WriteZGGHCount(const uint32_t& nCount);
    bool ReadZGGHCount(uint32_t& nCount);
