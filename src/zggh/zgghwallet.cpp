...
CzGGHWallet::CzGGHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zggh seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGGHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGGHSeed_deprecated()) {
                    LogPrintf("%s: Updated zGGH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zggh seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zggh wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGGHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGGHCount(nCountLastUsed);
    else if (!walletdb.ReadZGGHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGGHWallet::Lock()
{
    seedMaster = 0;
}

void CzGGHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGGHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGGH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGGHWallet::LoadMintPoolFromDB()
{
...
void CzGGHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGGHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGGHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgghTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGGHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGGH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgghTracker which also adds to database
    pwalletMain->zgghTracker->Add(dMint, true);
...
void CzGGHWallet::SeedToZGGH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGGHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGGHWallet::GenerateDeterministicZGGH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGGHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGGHWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzGGHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
