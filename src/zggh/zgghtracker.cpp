...
CzGGHTracker::CzGGHTracker(std::string strWalletFile)
{
...
CzGGHTracker::~CzGGHTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGGHTracker::Init()
{
...
bool CzGGHTracker::Archive(CMintMeta& meta)
{
...
bool CzGGHTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGGHTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGGHTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGGHTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzGGHTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzGGHTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzGGHTracker::GetSerialHashes()
{
...
CAmount CzGGHTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGGHTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGGHTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGGHTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGGHTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGGHTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGGHTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGGHTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGGHTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGGHTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGGHTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzGGHWallet* zGGHWallet)
{
    bool iszGGHWalletInitialized = (NULL != zGGHWallet);
    CMintMeta meta;
    ...
    if (! iszGGHWalletInitialized)
        zGGHWallet = new CzGGHWallet(strWalletFile);
    meta.isSeedCorrect = zGGHWallet->CheckSeed(dMint);
    if (! iszGGHWalletInitialized)
        delete zGGHWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzGGHTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGGHTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGGHTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGGHTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGGHTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGGHTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzGGHWallet* zGGHWallet = new CzGGHWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zGGHWallet);
        }
        delete zGGHWallet;
        LogPrint("zero", "%s: added %d dzggh from DB\n", __func__, listDeterministicDB.size());
    }
